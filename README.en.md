...
For `quantskills` navigation, automatic category overrides come only from canonical registry `category` enum values. Keyword-only matches are reported as maintainer-review suggestions and are not written to `categoryOverride`.
