...
For `quantskills` navigation, automatic category overrides come only from canonical registry `category` enum values. Business-signal matches are reported with rationale as maintainer-review suggestions and are not written to `categoryOverride`.
