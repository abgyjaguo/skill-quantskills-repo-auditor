...
7. Sync the organization homepage when needed.
   ...
   - During `--apply-index-updates`, registry generation runs before quantskills navigation generation. Registry categories and conservative public repository signals are used to update `quantskills/data/curation.json` so public `skill-*` and `agent-*` repositories can appear in the correct navigation category instead of falling into "Repos not in catalog".
...
7. Add `--apply-index-updates --local-root D:/quantskill` only after explicit maintainer approval to synchronize the GitHub organization homepage, registry artifacts, and quantskills navigation through local scripts.
   - This also synchronizes quantskills category overrides from registry metadata and conservative public skill/agent repository signals before generating navigation README files.
